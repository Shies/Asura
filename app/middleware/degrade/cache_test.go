...
func newMemcache() (*Cache, func()) {
	s := store2.NewMemcache(&conf.Memcache{
		Host: "127.0.0.1",
		Port: "11211",
	})
	return New(s), func() {}
...
func newFile() (*Cache, func()) {
	...
	s := store2.NewFile(&store2.FileConfig{
		RootDir: path,
	})
	remove := func() {
		os.RemoveAll(path)
...
func pageCase(cache *Cache, testExpire bool) func(t *testing.T) {
	return func(t *testing.T) {
		...
		engine.GET("/page-cache", cache.Cache(pc, nil), func(ctx *blade.Context) {
...
func pageMultiWriteCase(cache *Cache) func(t *testing.T) {
	return func(t *testing.T) {
		...
		pc := NewPage(3)
		engine := blade.Default()
		engine.GET("/page-cache-write", cache.Cache(pc, nil), func(ctx *blade.Context) {
			ctx.Writer.Header().Set("X-Hello", "World")
...
func degradeCase(cache *Cache) func(t *testing.T) {
	return func(t *testing.T) {
		...
		engine := blade.Default()
		engine.GET("/scheduled/error", cache.Cache(degrade.Args("name", "age"), nil), func(c *blade.Context) {
...
func controlCase(cache *Cache, testExpire bool) func(t *testing.T) {
	return func(t *testing.T) {
		...
		engine := blade.Default()
		engine.GET("/large/response", cache.Cache(control, filter), func(c *blade.Context) {
		...
		engine.GET("/large/response/error", cache.Cache(control, filter), func(c *blade.Context) {
